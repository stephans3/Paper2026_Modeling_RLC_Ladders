...
\begin{table}[b]
	...
	\begin{tabularx}{\columnwidth}{l|l|l}\hlinewd{1.2pt}
		...
		$R_{n}$, $L_{n}$, $C_{n}$ & Resistor, Inductor, Capacitor & $\mathbb{R}_{>0}$ \\ \hlinewd{1.2pt}
	\end{tabularx}
...
\end{equation}
where our coefficients $(a_{n}, b_{n})$ stand for the related electrical coefficients, $R_{n}$, $L_{n}$, $C_{n}$; and the finite difference stencil is
\begin{equation*}
	\tilde{F} = 
	\begin{pmatrix}
		1 & 0 & \cdots &  &  0\\
...
\end{equation*}
The matrix entries for each circuit type are noted in Table \ref{table:overview_matrix_entries}. The triangular matrix structure corresponds to a spatially discrete integration and the finite difference shape to a differentiation.
...
We specify the second upper triangular matrix as $U = \left[\alpha_{n}~b_{k}\right]$ analog to $T$ in  Eq. \eqref{eq:triangular_matrix}, with equal $b_{k}$ but different $a_{n} \neq \alpha_{n}$. We evaluate the multiplication elementwise as 
\begin{align}
...
\begin{equation}
	\left. \tilde{F}^{-1} \cdot \tilde{T}  \right\rvert_{n,k} = b_{k} \sum_{i=1}^{\min\{n,k\}} a_{i}
\end{equation}
...
We calculate the inverse Matrix $M^{-1}$ symbolically as
\begin{align*}
	M^{-1} = 
	\begin{pmatrix}
		\frac{1}{L_{1} ~ C_{1}} & \frac{-1}{L_{2} ~ C_{1}} & 0  						 & \cdots & 0 \\[1ex]
		0 						 &   \frac{1}{L_{2} ~ C_{2}} & \frac{-1}{L_{3} ~ C_{2}} &  & \vdots \\
		\vdots 				 		 &  \ddots 					 & \ddots 					 & \ddots & 0	 \\
		&  & 0 & \frac{1}{L_{N-1} ~ C_{N-1}} & \frac{-1}{L_{N} ~ C_{N-1}} \\[1ex]
		0  & \cdots 	& &  			0			   & \frac{1}{L_{N} ~ C_{N}} 
	\end{pmatrix}
\end{align*}
and we yield the  matrix entries as $P = [s_{n,k}]$ as
\begin{align*}
	\tilde{s}_{n,n} =&~ \frac{1}{L_{n} ~ C_{n}} + \frac{1}{L_{n+1} ~ C_{n}} \text{,} \\
	\tilde{s}_{n,n-1} =&~ \frac{-1}{L_{n} ~ C_{n}} \text{,} \quad \tilde{s}_{n,n+1} =~ \frac{-1}{L_{n+1} ~ C_{n}}
\end{align*}
and $Q = [d_{n,k}]$ as
\begin{align}
	\tilde{d}_{n,k} =
	\begin{cases}
		\frac{R_{n}}{L_{n}} \quad &\text{if} ~ n = k ~\text{,}\\[1ex]
		\left[\frac{R_{n}}{L_{n}} - \frac{R_{n+1}}{L_{n+1}} \right] \frac{C_{k}}{C_{n}}\quad &\text{if} ~ n < k ~\text{,}\\[1ex]
		0 \quad &\text{else}
	\end{cases}
\end{align}
with row $n\in \{1,\ldots,N\}$ and column $k\in \{1,\ldots,N\}$.


Matrix $P$ has a tridiagonal shape while $Q$ has in general an upper triangular shape. If all resistor and inductor values are identical as
\begin{equation*}
	R_{1} = R_{2} = \ldots = R_{N} ~\text{and}\quad L_{1} = L_{2} = \ldots = L_{N}~\text{,}
\end{equation*}
we yield a diagonal matrix  $Q = \frac{R}{L}~I$.



\section{Eigenvalues and Simulation of RLC Ladder}

\begin{itemize}
	\item Constant coefficients
	\item Increasing coefficients
	\item Realistic coefficients + noise (deviation)
\end{itemize}


\begin{equation*}
	u(t) = \sin\left(c~\pi \sqrt{t+\epsilon}\right)
\end{equation*}
alternative
\begin{equation*}
	u(t) = \sin\left(1 + 1000~\tanh(10 (\frac{t}{1000} - \frac{1}{2}))\right)
\end{equation*}

\begin{equation*}
	u(t) = \sin\left(1000\pi~\tanh(0.001~t)\right)
\end{equation*}

\begin{equation*}
	u(t) = \sin\left(k\pi~\tanh(t/k)\right) \quad \text{with} \quad k=1000
\end{equation*}
...
\begin{equation*}
	M = 
	\begin{pmatrix}
		L_{1} C_{1} & L_{1} C_{2} & \cdots &  L_{1} C_{N-1} & L_{1} C_{N} \\
		0 			& L_{2} C_{2} & \cdots &  L_{2} C_{N-1} & L_{2} C_{N} \\
		\vdots			& \ddots	  & \ddots &  \vdots        & 	\vdots	  \\
		&			  &        & L_{N-1} C_{N-1}& L_{N-1} C_{N} \\
		0			&	\cdots		  &        &	0            & L_{N} C_{N} 
	\end{pmatrix} \text{,}
\end{equation*}
\begin{equation*}
	D = 
	\begin{pmatrix}
		R_{1} C_{1} & R_{1} C_{2} & \cdots &  R_{1} C_{N-1} & R_{1} C_{N} \\
		0 			& R_{2} C_{2} & \cdots &  R_{2} C_{N-1} & R_{2} C_{N} \\
		\vdots & \ddots	  & \ddots &  \vdots        & 	\vdots	  \\
		&			  &        & R_{N-1} C_{N-1}& R_{N-1} C_{N} \\
		0 & \cdots			  &        & 0	            & R_{N} C_{N} 
	\end{pmatrix}
\end{equation*}
and
...
\section{Matrix Entries}
\subsection{Capacitor as Shunt}

and the matrix entries
\begin{align}
	m_{n,k} = 
	\begin{cases}
		L_{n}~C_{k} \quad &\text{if} ~ n \leq k ~\text{,}\\[1ex]
		0 \quad &\text{else, }
	\end{cases} \\
	d_{n,k} = 
	\begin{cases}
		R_{n}~C_{k} \quad &\text{if} ~ n \leq k ~\text{,}\\[1ex]
		0 \quad &\text{else}
	\end{cases}
\end{align}
and 
\begin{align*}
	s_{n,k} = 
	\begin{cases}
		1 \quad &\text{if} ~ n = k ~\text{,}\\[1ex]
		-1 \quad &\text{if} ~ n = k+1 ~\text{,}\\[1ex]
		0 \quad &\text{else.}
	\end{cases}
\end{align*}
...
\begin{align*}
	m_{n,k} = 
	\begin{cases}
		\frac{L_{n}}{R_{k}} \quad &\text{if} ~ n \leq k ~\text{,}\\[1ex]
		0 \quad &\text{else}
	\end{cases}
\end{align*}
...
\begin{align*}
	s_{n,k} = 
	\begin{cases}
		\left[R_{k} C_{n} \right]^{-1} \quad &\text{if} ~ n \leq k ~\text{,}\\[1ex]
		0 \quad &\text{else}
	\end{cases}
\end{align*}
