...
%\begin{enumerate}
%	\item Simple circuits
%	\item Control theory: modeling of transfer functions and state space models
%	\item Inductors and Capacitors as energy storage: first-order and second-order systems
%	\item Concatenation of simple circuits increases number of energy storages and model order and it leads to a ladder 
%	\item Modern approaches to model large circuits ... modified nodal analysis, see \cite{book:najm2010circuit} \cite{article:tischendorf1998topological,article:marz2003finding}
%	\item MNA and port-Hamiltonian modeling \cite{article:gunther2020dynamic, article:gernandt2021porthamiltonian}
%\end{enumerate}
...
%like, fundamentals of physics, electrical engineering and control theory. These circuits are not limited to describe an electrical system, but also emphasize connections to other scientific domains, e.g. to study wave phenomena {\color{red} CITE!!} or more abstract models like port-Hamiltonian systems {\color{red} CITE!!}. In control and systems theory such simple exemplify the modeling of first-order and second-order systems via transfer functions.


%Modeling of circuits and the numerical analysis of the resulting systems in  , i.e. with modified nodal analysis 
%in the book \cite{book:najm2010circuit}
%
%The modeling of electrical ladder circuits also connected to machine learning like data-driven model discovery {\color{red} CITE!!} and dynamic mode decomposition \cite{article:peters2023dynamic}.
%
%machine learning techniques connected with the aforementioned modified nodal analysis 
%\cite{article:galetzka2023data}, port-Hamiltonian modeling \cite{article:neary2025neural} and ... (Daniel: DMD)

%classes, e.g. physics to academic courses, e.g. control eng.. Not only for to describe the electrical behavior, but also emphasize connections to other scientific domains, e.g. waves. Single circuits in control eng. and system theory are used to exemplify the modeling via transfer functions. For simple circuits this approach is easy, but it becomes more difficult for larger circuits. They are used to introduce electronic filters, RC is low-pass; Bode plot.
...
\begin{itemize}
	\item Instead of $R$ and $L$ maybe $\ell$ $r$ $\mathcal{c}$
...
\begin{equation}
A_{I} = -M^{-1}~S \quad \text{and} \quad A_{II} := -M^{-1}~D	\label{eq:first_order_submatrices_P_Q}
\end{equation}
...
\begin{align*}
	...
	\end{pmatrix} \text{.}
\end{align*}
A multiplication of $\mathbf{T}^{-1}$ with a lower finite difference stencil, see Fig. \ref{fig:matrix_multiplication} (a), results in a tridiagonal shaped matrix as in Eq. \eqref{eq:finite_diff_stencil} with a $n$-th row vector elements 
\begin{equation*}
...
\end{equation}
we yield a Laplacian matrix (side)
\begin{equation}
...
\begin{equation}
	\mathbf{L} = \mathbf{T}^{-1} ~ \tilde{F} % Laplacian matrix
\end{equation}

\begin{equation}
	\mathbf{U} = \mathbf{T}^{-1} ~ \mathbf{T} % Triangular matrix
\end{equation}

\begin{equation}
	\mathbf{W} = \tilde{F}^{-1} ~ \mathbf{T} % Full / dense matrix
\end{equation}
...
\begin{equation}
	P = \begin{cases}
		\mathbf{T}^{-1} ~ \tilde{F} \text{,} \quad \text{for circuit type (a),} \\
		\mathbf{T}^{-1} ~ \mathbf{T} \text{,} \quad \text{for circuit type (b),} \\
		\tilde{F}^{-1} ~ \mathbf{T} \text{,} \quad \text{for circuit type (c)}
	\end{cases}
\end{equation}

\begin{equation}
	Q = \begin{cases}
		\mathbf{T}^{-1} ~ \mathbf{T} \text{,} \quad \text{for circuit type (a),} \\
		\mathbf{T}^{-1} ~ \tilde{F} \text{,} \quad \text{for circuit type (b),} \\
		\tilde{F}^{-1} ~ \mathbf{T} \text{,} \quad \text{for circuit type (c)}
	\end{cases}
\end{equation}
...
\begin{equation}
	P_{\alpha,\beta} := - \mathbf{T}_{a,b}^{-1} ~ \cdot ~ \mathbf{T}_{\tilde{a},\tilde{b}}
\end{equation}
...
\begin{equation}
	\alpha_{n} = \frac{\tilde{a}_{n}}{a_{n}} \quad \text{and} \quad \beta_{n,k} = \left[\frac{\tilde{a}_{n}}{a_{n}} - \frac{\tilde{a}_{n+1}}{a_{n+1}} \right] \frac{\tilde{b}_{k}}{b_{n}}
\end{equation}
for 

We consider the inverse triangular matrix $\mathbf{T}^{-1}$ as above and we specify the second upper triangular matrix as $U = \left[\alpha_{n}~b_{k}\right]$ analog to $T$ in  Eq. \eqref{eq:triangular_matrix}, with equal $b_{k}$ but different $a_{n} \neq \alpha_{n}$. We evaluate the multiplication elementwise as 
\begin{align}
	\left. \mathbf{T}^{-1} \cdot U \right\lvert_{n,k} = 
	\begin{cases}
...
\begin{equation*}
	\mathbf{T}^{-1} \cdot U = \epsilon~I \text{.}
\end{equation*}
...
\end{equation}
where $\mathbf{Q} = [q_{n,k}]$ and 
\begin{equation}
	q_{n,k} = b_{k} \sum_{i=1}^{\min\{n,k\}} a_{i}
\end{equation}
for $n,k \in \{1,\ldots,N\}$.

\begin{equation}
	\mathbf{Q} = 
	\def\aa{\color{red}\alpha_{2}}
	\def\ab{\color{red}\alpha_{N-1}}
	\begin{pmatrix}
		\alpha_{1} & \alpha_{1} & \alpha_{1} & \ldots & \alpha_{1} & \alpha_{1} \\
		\alpha_{1} & \aa & \aa & \ldots & \aa & \aa \\
		     & \aa & \alpha_{3} & \ldots & \alpha_{3} & \alpha_{3} \\
		\vdots     & \vdots & \vdots & \ddots \\
		\alpha_{1}& \aa & \alpha_{3} & & \ab & \ab \\
		\alpha_{1} & \aa & \alpha_{3} & & \ab & \alpha_{N}
	\end{pmatrix}
\end{equation}
...
\begin{equation*}
	\tilde{F}^{-1} =
	 ...
	 \end{pmatrix} \text{.}
\end{equation*}
We multiply this lower triangular matrix with $\mathbf{T}$ as in Eq. \eqref{eq:triangular_matrix} and we evaluate the results elementwise as
\begin{equation}
	\left. \tilde{F}^{-1} \cdot \mathbf{T}  \right\rvert_{n,k} = b_{k} \sum_{i=1}^{\min\{n,k\}} a_{i} \text{.}
\end{equation}
If all values of $b_{n}$ are equal, then this matrix multiplication results in a shape
\begin{equation*}
	\tilde{F}^{-1} \cdot \mathbf{T} =
	\begin{pmatrix}
		 & \text{---} & v_{1} & \text{---} & \\
		| & & \text{---} & v_{2} & \text{---} \\
		v_{1} & | & & &  \\
...
\begin{enumerate}
	\item Describing circuits (Done)
	\item 2nd order system model: matrices + input signals (Done)
	\item 1st order: matrices (P,Q), 2nd order diff stencil, ..   
	\item output signal 
\end{enumerate}
...
\begin{figure*}[t!]
	\centering
	\subfloat[Input Signals]{\includegraphics[width=0.99\columnwidth]{images/input_signal}}\quad
	\subfloat[Capacitor]{\includegraphics[width=0.99\columnwidth]{images/lines_shunt_capacitor}}
	
	\subfloat[Resistor]{\includegraphics[width=0.99\columnwidth]{images/lines_shunt_resistor}}\quad
	\subfloat[Inductor]{\includegraphics[width=0.99\columnwidth]{images/lines_shunt_inductor}}
...
% D - 2nd order ODE
% E - nein
% F - Finite Diff Stencil
% G - 2nd order ODE
% H
% I - Einheitsmatrix
% J
% K
% L - Laplacian matrix: inv(T)~F
% M - 2nd order ODE
% N
% O - nein
% P - 1st order ODE
...
\begin{thebibliography}{99}
	{\small\fontsize{9.4}{1.2em}\selectfont
		\setlength{\parskip}{-0.5mm}
		\addcontentsline{toc}{section}{References}\vspace{-3mm}
		%
...
%		\bibitem{ref_1}\raggedright
%		Author, A. {\itshape Title of his published book}. Edition. City of
%		Publisher: Abbrev. of Publisher; year. total pages of
%		book p.
%		\bibitem{ref_2}\raggedright
%		Author2 A, Author3 BC, Author4 D, editors. {\itshape Title of
%			published book}. 8th ed. City of Publisher: Abbrev. of
%		Publisher; year. 1234 p.
%		\bibitem{ref_3}\raggedright
%		Author2 B, Author3 CA. Title of the chapter within the
%		book. In: Author4 D, Author5 E, editors. {\itshape Title of published Book}. 
%		Edition. City of Publisher: Abbrev. of Publisher;
%		year. p 1234--total pages of chapter.
%		\bibitem{ref_4}\raggedright
%		Author, A. {\itshape Title of Dissertation} [dissertation]. 
%		[Departement, (abrev. State)]. Name of University; year. doi: doi
%		number.
%		\bibitem{ref_5}\raggedright
%		Author1 AB, Author2 C. Title of paper. In Editor1 A,
%		Editor2 B, editors. Title of book. {\itshape Number and Name of
%			the Conference}; 2014 Jan; Place of Conference. Place of
%		Publication: publisher. page-page. doi: doi number.
%		\bibitem{ref_6} \raggedright
%		Author A, Author2 B. Title of Article. {\itshape Journal Title ab-
%			breviation}. year; volume(issue): page--page. doi: doi
%		number.
	}
\end{thebibliography}
